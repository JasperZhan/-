...
#include "child_parent_tree_node.h"									// 孩子双亲表示森林结点类模板
...
public:
//  森林方法声明及重载编译系统默认方法声明:
	ChildParentForest();											// 无参数的构造函数模板
	virtual ~ChildParentForest();									// 析构函数模板
	...
	ChildParentForest(const ChildParentForest<ElemType> &copy);		// 复制构造函数模板
